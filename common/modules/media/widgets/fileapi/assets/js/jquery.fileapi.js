...
(function ($, api) {
	...
	var noop = $.noop;
	var oldJQ = !$.fn.prop;
	var propFn = oldJQ ? 'attr' : 'prop';
	var removePropFn = oldJQ ? 'removeAttr' : 'removeProp';
	var _dataAttr = 'data-fileapi';
	var _dataFileId = 'data-fileapi-id';
	var _slice = [].slice;
	var _each = api.each;
	var _extend = api.extend;

	var _bind = function (ctx, fn) {
	...
	var _optDataAttr = function (name) {
		return '[' + _dataAttr + '="' + name + '"]';
	}

	var _isID = function (selector) {
	...
	var Plugin = function (el, options) {
		...
		this.$el.on('reset.fileapi', _bind(this, this._onReset));
		this.$el.on('submit.fileapi', _bind(this, this._onSubmit));
		this.$el.on('upload.fileapi progress.fileapi complete.fileapi', _bind(this, this._onUploadEvent));
		this.$el.on('fileupload.fileapi fileprogress.fileapi filecomplete.fileapi', _bind(this, this._onFileUploadEvent));
		this.$el.on('click', '[' + _dataAttr + ']', _bind(this, this._onActionClick));
	...
	Plugin.prototype = {
		...
		_getFile: function (uid) {
			return api.filter(this.files, function (file) {
				return api.uid(file) == uid;
			})[0];
		},
		...
		crop: function (file, coords) {
			...
			var preview = opts.multiple ? this.option('elements.file.preview') : opts.elements.preview;
			var $el = (opts.multiple ? this.$file(uid) : this.$el).find(preview && preview.el);
				...
				api.getInfo(file, _bind(this, function (err, info) {
					if (err) {
						...
						if (!$el.find('div>div').length) {
							$el.html($('<div><div></div></div>').css(preview).css('overflow', 'hidden'));
						}
		...
		remove: function (file) {
			...
			this.queue = api.filter(this.queue, function (file) {
				return api.uid(file) != uid;
			});
			this.files = api.filter(this.files, function (file) {
				return api.uid(file) != uid;
			});
		...
		destroy: function () {
			...
			this.$el.off('.fileapi').removeData('fileapi');
	...
	$.fn.jCropper = function (opts) {
		...
		if (typeof opts === 'string') {
			...
			var minSize = opts.minSize || [0, 0];
			var ratio = (opts.aspectRatio || minSize[0] / minSize[1]);
			...
			api.getInfo(file, function (err, info) {
				...
				if (maxSize) {
					Image.resize(Math.max(maxSize[0], minSize[0]), Math.max(maxSize[1], minSize[1]), 'max');
				}

				Image.rotate(deg === void 0 ? 'auto' : deg).get(function (err, img) {

					if (img.width < opts.minSize.width)
						img.width = opts.minSize.width;

					var selection = opts.selection;
					var minSide = Math.min(img.width, img.height);
					var selWidth = minSide;
					var selHeight = minSide / ratio;
					var deg = FileAPI.Image.exifOrientation[info.exif && info.exif.Orientation] || 0;
					...
					if (selection) {
						...
						var selLeft = (img.width - selWidth) / 2;
						var selTop = (img.height - selHeight) / 2;
					...
					_each(['onSelect', 'onChange'], function (name, fn) {
						if (fn = opts[name]) {
							opts[name] = function (coords) {
								...
								var ow = info.width;
								var oh = info.height;
								var fx = coords.x / img.width;
								var fy = coords.y / img.height;
								var fw = coords.w / img.width;
								var fh = coords.h / img.height;
								var x = ow * (flip ? fy : fx);
								var y = oh * (flip ? 1 - (coords.x + coords.w) / img.width : fy);
								var w = ow * (flip ? fh : fw);
								var h = oh * (flip ? fw : fh);
								...
								fn({
									...
									y: y,
									w: w,
									h: h, rx: fx * (flip ? oh : ow),
									ry: fy * (flip ? ow : oh), rw: fw * (flip ? oh : ow),
									rh: fh * (flip ? ow : oh),
									lx: coords.x,
									ly: coords.y,
									lw: coords.w,
					...
					$el.closest('.modal-dialog').width($inner.width() + 30);
				});
			});
		}
	...
	$.fn.mCropper = function (opts) {
		...
		api.getInfo(file, function (err, info) {
			...
			Image.rotate(deg === void 0 ? 'auto' : deg).get(function (err, img) {
				...
				opts.crop = function(e) {

					var flip = deg % 180;
					var ow = info.width;
					var oh = info.height;
					var fx = e.x / img.width;
					var fy = e.y / img.height;
					var fw = e.width / img.width;
					var fh = e.height / img.height;
					var x = ow * (flip ? fy : fx);
					var y = oh * (flip ? 1 - (e.x + e.w) / img.width : fy);
					var w = ow * (flip ? fh : fw);
					var h = oh * (flip ? fw : fh);
