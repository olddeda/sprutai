...
?>
	<div id="<?= $selector; ?>" class="uploader uploader-single" data-width="<?= $width ?>" data-height="<?= $height ?>" data-format="<?= MediaFormat::format($width, $height, $format) ?>">
		...
		<?php if ($preview === true) { ?>
			...
			<div class="uploader-preview">
				<div class="uploader-preview-placeholder thumbnail" style="height:<?= $height + 10 ?>px;">
					<?php if ($fileExists) { ?>
						<?= Html::img($media->getImageSrc($width, $height, $format)) ?>
					<?php } ?>
					<div class="uploader-preview-placeholder-wrapper <?= ($fileExists) ? 'hidden' : '' ?>">
						<span class="uploader-preview-placeholder-icon glyphicon glyphicon-picture" style="font-size:<?= (($height + 10) - ceil((($height + 10) / 100 * 35))) ?>px;"></span>
					</div>
				</div>
		...
		<div class="uploader-actions js-fileapi-wrapper">
			<div class="uploader-browse btn btn-primary" data-fileapi="active.hide">
				<span data-fileapi="browse-text" class="browse-text">
                    <?= Yii::t('media-fileapi', 'button_select_image') ?>
                </span>
