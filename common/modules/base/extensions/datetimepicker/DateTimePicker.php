...
class DateTimePicker extends InputWidget
{
	...
	public function init()
	{
		...
		if (!$this->language)
			$this->language = strtolower(current(explode('-', Yii::$app->language)));
	}
	...
	public function run() {
		...
		if (!$this->inline) {
			...
			$pickerAddon = Html::tag('span', $pickIcon, ['class' => 'input-group-addon-picker input-group-addon']);
		}
		else {
		...
		if (strpos($this->template, '{button}') !== false || $this->inline) {
			$input = Html::tag('div', strtr($this->template, ['{input}' => $input, '{reset}' => $resetAddon, '{button}' => $pickerAddon]), $this->containerOptions);
		}
		echo $input;
	...
	public function registerClientScript() {
		...
		$js[] = "$(document).on('pjax:beforeSend' , function(event) { $selector.datetimepicker('hide'); })";
