<?php
namespace common\modules\base\extensions\rss;
...
class Feed extends DOMDocument
{
	...
	public function addChannel($href = null) {
		...
		$this->addChannelElement('atom:link', '', [
			'href' => $href ? $href : $_SERVER['REQUEST_SCHEME'].'://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'],
			'rel' => 'self',
	...
	public function addChannelElement($element, $value, $attr = []) {
	...
	public function addChannelCategory($value, $domain = null) {
		return $domain ? $this->addChannelElement('category', $value, ['domain' => $domain]) : $this->addChannelElement('category', $value);
	}
	...
	public function addItemElement($element, $value, $attr = [], $cdata = false) {
		if ($cdata) {
			$element = $this->createElement($element, '');
	...
	public function addItemDescription($value) {
		return $this->addItemElement('description', $value, [], true);
	}
	
	public function addItemContent($value) {
		return $this->addItemElement('content:encoded', $value, [], true);
	}
	...
	public function addItemCategory($value, $domain = null) {
		return $domain ? $this->addItemElement('category', $value, ['domain' => $domain]) : $this->addItemElement('category', $value);
	}
	...
	public function __toString() {
		...
		return $this->saveXML((new \DOMXPath($this))->query('/')->item(0));
	}
	...
	private function normalizeCdata($value) {
		return '<![CDATA['.$value.']]>';
	}
}