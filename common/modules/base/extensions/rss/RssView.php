...
class RssView extends BaseListView
{
	 ...
	 * @var array
	 */
	public $requiredChannelElements = [
		'title',
		'link',
		'description'
	];
	...
	public function renderItem($model) {
		...
		foreach ($this->items as $element => $value) {
			if (is_string($value)) {
				...
				$methodName = 'addItem'.ucfirst($element);
				if (method_exists($this->getFeed(), $methodName)) {
					call_user_func([$this->getFeed(), $methodName], $result);
				}
