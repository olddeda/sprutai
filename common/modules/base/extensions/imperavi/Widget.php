...
class Widget extends BaseWidget
{
	...
	const INLINE_JS_KEY = 'common/modules/media/widgets/imperavi/';
	...
	public function init() {
		...
		if ($request->enableCsrfValidation) {
			$this->settings['imageData'][$request->csrfParam] = $request->getCsrfToken();
	...
	public static function registerTranslations() {
		if (!isset(Yii::$app->i18n->translations['imperavi']) && !isset(Yii::$app->i18n->translations['imperavi*'])) {
			Yii::$app->i18n->translations['common/base/extensions/imperavi'] = [
				'class' => 'yii\i18n\PhpMessageSource',
				'basePath' => '@common/modules/base/extensions/imperavi/messages',
				'forceTranslation' => true,
				'fileMap' => [
					'imperavi' => 'imperavi.php'
				]
			];
		}
	}
	...
	protected function registerClientScripts() {
		...
		$view->registerJs('$R('.$selector.', '.$settings.');', $view::POS_READY, self::INLINE_JS_KEY.$this->options['id']);
	}
	...
	protected function registerDefaultCallbacks() {
		$id = $this->options['id'];
		$js = new JsExpression("function (e) {
		...
		$this->settings['callbacks']['keydown'] = $js;
		$this->settings['callbacks']['keyup'] = $js;
		$this->settings['callbacks']['blur'] = $js;
