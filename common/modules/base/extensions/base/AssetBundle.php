...
class AssetBundle extends \yii\web\AssetBundle
{
	...
	const WIDGET_ASSET = 'W3/@$$3T$';
	const WIDGET_PATH = 'W3/P@T#';

	public $js = self::WIDGET_ASSET;
	public $css = self::WIDGET_ASSET;
	public $sourcePath = self::WIDGET_PATH;
	public $depends = [
		'yii\web\JqueryAsset',
		'yii\web\YiiAsset',
	...
	public function init() {
		parent::init();
		if ($this->js === self::WIDGET_ASSET) {
			$this->js = [];
		}
		if ($this->css === self::WIDGET_ASSET) {
			$this->css = [];
		}
		if ($this->sourcePath === self::WIDGET_PATH) {
	...
	protected function setupAssets($type, $files = []) {
		if ($this->$type === self::WIDGET_ASSET) {
			$srcFiles = [];
			$minFiles = [];
