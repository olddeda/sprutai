<?php
namespace common\modules\base\widgets\favicon;
...
/**
 * Class Favicon
 * @package common\modules\base\widgets\favicon
 */
class FaviconWidget extends Widget
{
	...
	protected function generateFavicon() {
		...
		foreach ([16, 32, 96, 194] as $s) {
			$filename = sprintf('favicon-%sx%s.png', $s, $s);
			Image::getImagine()->open($favicon_path)->resize(new Box($s, $s))->save($path.DIRECTORY_SEPARATOR.$filename);
		}
	}
	...
	protected function generateAndroid() {
		...
		foreach ($variants as $conf) {
			...
			$Icon = Image::getImagine()->open($favicon_path)->resize($resize);
			
			Image::getImagine()->create($canvas, (new RGB)->color('#ffffff'))->paste($Icon, $point)->save($file);
		}
	}
	...
	protected function generateApple() {
		...
		$Color = empty($this->fillColor) ? (new RGB)->color('#ffffff') : (is_string($this->fillColor) ? (new RGB)->color($this->fillColor) : $this->color);
		
		foreach ([57, 60, 72, 76, 114, 120, 144, 152, 180] as $s) {
			$Icon = Image::getImagine()->open($favicon_path)->resize(new Box($s, $s));
			
			Image::getImagine()->create(new Box($s, $s), $Color)->paste($Icon, $Point)->save($path.DIRECTORY_SEPARATOR.sprintf('apple-touch-icon-%sx%s.png', $s, $s));
			
			if ($default === $s) {
				Image::getImagine()->create(new Box($s, $s), $Color)->paste($Icon, $Point)->save($path.DIRECTORY_SEPARATOR.'apple-touch-icon.png');
			}
		}
	}
	...
	protected function generateMicrosoftTiles() {
		...
		Image::getImagine()->open($favicon_path)->resize(new Box(144, 144))->save($path.DIRECTORY_SEPARATOR.sprintf('mstile-%sx%s.png', 144, 144));
	...
	protected function generateBrowserConfigXml() {
		...
		file_put_contents($this->getWebPath().DIRECTORY_SEPARATOR.'browserconfig.xml', $this->getViewContent('browserconfig.xml'));
	}
	...
	protected function generateManifestJson() {
		...
		file_put_contents($this->getWebPath().DIRECTORY_SEPARATOR.'manifest.json', $this->getViewContent('manifest.json'));
	}
	...
	protected function getViewContent($filename, $params = []) {
		...
		$replace_keys = array_map(function ($val) {
			return sprintf('${%s}', $val);
		}, array_keys($replace));
		$replace_values = array_values($replace);
		return str_replace($replace_keys, $replace_values, file_get_contents($filepath));
	}
	...
	protected function getFaviconPath() {
		...
		if (empty($favicon_path) || !file_exists($favicon_path) || !is_file($favicon_path)) {
			throw new InvalidConfigException('Favicon file not exists.');
		}
		
		if (!is_readable($favicon_path)) {
			throw new InvalidConfigException('Favicon file is not readable.');
		}
		
		$size = @getimagesize($favicon_path);
		
		if (empty($size) || $size === false) {
			throw new InvalidConfigException('Favicon file is not an image.');
		}
		
		if ($size['mime'] !== 'image/png') {
			throw new InvalidConfigException('Favicon file must be a png image format.');
		}
	...
	protected function getWebPath() {
		...
		if (empty($path) || !file_exists($path) || !is_dir($path)) {
			throw new InvalidConfigException('Web path not exists.');
		}
		
		if (!is_writable($path)) {
			throw new InvalidConfigException('Web path is not writable.');
		}
