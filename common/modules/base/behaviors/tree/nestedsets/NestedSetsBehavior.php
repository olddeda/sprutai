...
class NestedSetsBehavior extends Behavior
{
	...
	public function getParents($depth = null) {
		...
		$condition = [
			...
			[
				'<',
				"{$tableName}.[[{$this->leftAttribute}]]",
				$this->owner->getAttribute($this->leftAttribute)
			],
			[
				'>',
				"{$tableName}.[[{$this->rightAttribute}]]",
		...
		if ($depth !== null) {
			$condition[] = [
				...
				$this->owner->getAttribute($this->depthAttribute) - $depth,
			];
		}
		
		$query = $this->owner->find()->andWhere($condition)->andWhere($this->treeCondition())->addOrderBy(["{$tableName}.[[{$this->leftAttribute}]]" => SORT_ASC]);
		$query->multiple = true;
	...
	public function getParent() {
		$tableName = $this->owner->tableName();
		$query = $this->getParents(1)->orderBy(["{$tableName}.[[{$this->leftAttribute}]]" => SORT_DESC])->limit(1);
	...
	public function getRoot() {
		$tableName = $this->owner->tableName();
		$query = $this->owner->find()->andWhere(["{$tableName}.[[{$this->leftAttribute}]]" => 1])->andWhere($this->treeCondition())->limit(1);
		$query->multiple = false;
	...
	public function getDescendants($depth = null, $andSelf = false, $backOrder = false) {
		...
		$query = $this->owner->find()->andWhere($condition)->andWhere($this->treeCondition())->addOrderBy(["{$tableName}.[[{$attribute}]]" => $backOrder ? SORT_DESC : SORT_ASC]);
		$query->multiple = true;
	...
	public function getLeaves($depth = null) {
		$tableName = $this->owner->tableName();
		$query = $this->getDescendants($depth)->andWhere(["{$tableName}.[[{$this->leftAttribute}]]" => new Expression("{$tableName}.[[{$this->rightAttribute}]] - 1")]);
		$query->multiple = true;
	...
	public function getPrev() {
		$tableName = $this->owner->tableName();
		$query = $this->owner->find()->andWhere(["{$tableName}.[[{$this->rightAttribute}]]" => $this->owner->getAttribute($this->leftAttribute) - 1])->andWhere($this->treeCondition())->limit(1);
		$query->multiple = false;
	...
	public function getNext() {
		$tableName = $this->owner->tableName();
		$query = $this->owner->find()->andWhere(["{$tableName}.[[{$this->leftAttribute}]]" => $this->owner->getAttribute($this->rightAttribute) + 1])->andWhere($this->treeCondition())->limit(1);
		$query->multiple = false;
	...
	public function beforeInsert() {
		...
		switch ($this->operation) {
			...
			default:
				throw new NotSupportedException('Method "'.$this->owner->class.'::insert" is not supported for inserting new nodes.');
		}
	}
	...
	protected function shift($from, $to, $delta, $tree = null) {
		if ($delta !== 0 && ($to === null || $to >= $from)) {
			...
			foreach ([
				...
				$this->owner->updateAll([$attribute => new Expression("[[{$attribute}]]".sprintf('%+d', $delta))], [
					'and',
					$to === null ? [
						'>=',
						$attribute,
	...
	protected function treeCondition() {
		...
		$attrs = $this->treeConditionAttributes();
		
		if (!is_null($this->treeAttribute)) {
			$attrs["{$tableName}.[[{$this->treeAttribute}]]"] = $this->owner->getAttribute($this->treeAttribute);
		}
	 ...
	 * @return array
	 */
	protected function treeConditionAttributes() {
		$tmp = [];
		foreach ($this->conditionAttributes as $attr) {
			$tmp[$attr] = $this->owner->getAttribute($attr);
		}
		return $tmp;
	}
}
