...
use common\modules\base\behaviors\sortable\SortableBehavior;
...
class AdjacencyListBehavior extends Behavior
{
	...
	public function getParents($depth = null) {
		...
		$query = $this->owner->find()->andWhere(["{$tableName}.[[".$this->getPrimaryKey()."]]" => $ids]);
		$query->multiple = true;
	...
	public function getRoot() {
		...
		$query = $this->owner->find()->andWhere(["{$tableName}.[[".$this->getPrimaryKey()."]]" => $id]);
	...
	public function getLeaves($depth = null) {
		$query = $this->getDescendants($depth)->joinWith([
				'children' => function ($query) {
					/** @var \yii\db\ActiveQuery $query */
					$modelClass = $query->modelClass;
					$query->from($modelClass::tableName().' children')->orderBy(null);
				}
	...
	public function getPrev() {
		...
		$tableName = $this->owner->tableName();
		$query = $this->owner->find()->andWhere([
				'and',
				["{$tableName}.[[{$this->parentAttribute}]]" => $this->owner->getAttribute($this->parentAttribute)],
				[
					'<',
					"{$tableName}.[[{$this->behavior->sortAttribute}]]",
					$this->owner->getSortablePosition()
				],
			])->orderBy(["{$tableName}.[[{$this->behavior->sortAttribute}]]" => SORT_DESC])->limit(1);
		$query->multiple = false;
	...
	public function getNext() {
		...
		$query = $this->owner->find()->andWhere([
			...
			])->orderBy(["{$tableName}.[[{$this->behavior->sortAttribute}]]" => SORT_ASC])->limit(1);
		$query->multiple = false;
	...
	public function getParentsIds($depth = null, $cache = true) {
		...
		while ($parentId !== null && ($depth === null || $depthCur < $depth)) {
			$query = (new Query())->select(["lvl0.[[{$this->parentAttribute}]] AS lvl0"])->from("{$tableName} lvl0")->where(["lvl0.[[{$primaryKey}]]" => $parentId]);
			for ($i = 0; $i < $this->parentsJoinLevels && ($depth === null || $i + $depthCur + 1 < $depth); $i++) {
				$j = $i + 1;
				$query->addSelect(["lvl{$j}.[[{$this->parentAttribute}]] as lvl{$j}"])->leftJoin("{$tableName} lvl{$j}", "lvl{$j}.[[{$primaryKey}]] = lvl{$i}.[[{$this->parentAttribute}]]");
			}
	...
	public function getDescendantsIds($depth = null, $flat = false, $cache = true) {
		...
		while (!empty($lastLevelIds) && ($depth === null || $depthCur < $depth)) {
			$levels = 1;
			$depthCur++;
			$query = (new Query())->select(["lvl0.[[{$primaryKey}]] AS lvl0"])->from("{$tableName} lvl0")->where(["lvl0.[[{$this->parentAttribute}]]" => $lastLevelIds]);
			if ($this->sortable !== false) {
			...
			for ($i = 0; $i < $this->childrenJoinLevels && ($depth === null || $i + $depthCur + 1 < $depth); $i++) {
				...
				$levels++;
				$j = $i + 1;
				$query->addSelect(["lvl{$j}.[[{$primaryKey}]] as lvl{$j}"])->leftJoin("{$tableName} lvl{$j}", [
						'and',
	...
	public function populateTree($depth = null) {
		...
		if ($depth === null) {
			...
			$nodes = $this->getDescendants($depth)->orderBy($this->sortable !== false ? [$this->behavior->sortAttribute => SORT_ASC] : null)->all();
		}
	...
	public function afterDelete() {
		if ($this->operation !== static::OPERATION_DELETE_ALL) {
			$this->owner->updateAll([$this->parentAttribute => $this->owner->getAttribute($this->parentAttribute)], [$this->parentAttribute => $this->owner->getPrimaryKey()]);
		}
		$this->operation = null;
	}
