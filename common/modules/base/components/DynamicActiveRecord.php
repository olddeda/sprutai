...
class DynamicActiveRecord extends ActiveRecord
{
	...
	public function dotAttributes() {
		return array_merge($this->attributes, static::dotKeyValues(static::dynamicColumn(), $this->_dynamicAttributes));
	}
	...
	public static function encodeForMaria($value) {
		return is_string($value) && (!mb_check_encoding($value, 'UTF-8') || strpos($value, self::DATA_URI_PREFIX) === 0) ? self::DATA_URI_PREFIX.base64_encode($value) : $value;
	}
	...
	public static function decodeForMaria($value) {
		return is_string($value) && strpos($value, self::DATA_URI_PREFIX) === 0 ? file_get_contents($value) : $value;
	}
	...
	public static function dynColDecode($encoded) {
		...
		$encoded = preg_replace_callback('/[\x00-\x1f]/', function ($matches) {
