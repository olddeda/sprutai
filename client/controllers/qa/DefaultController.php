<?php
namespace client\controllers\qa;
...
use client\models\Question;
use client\models\QuestionSearch;
use client\models\Answer;
...
class DefaultController extends Controller
{
	...
	public function actionPopular()  {
		$searchModel = new QuestionSearch();
		$dataProvider = $searchModel->search(Yii::$app->request->getQueryParams());
		$dataProvider->query->orderBy(['votes' => SORT_DESC]);
		$models = $dataProvider->getModels();
	...
	public function actionView($id) {
		...
		if ($model) {
			...
			/** @var Answer $model */
			$answer = Answer::find()->where('question_id = :question_id AND user_id = :user_id AND status = :status', [
				':question_id' => $id,
				':user_id' => Yii::$app->user->id,
				':status' => AnswerInterface::STATUS_DRAFT,
			])->one();
			if (is_null($answer)) {
				$answer = new Answer();
				$answer->question_id = $id;
				$answer->user_id = Yii::$app->user->id;
				$answer->status = AnswerInterface::STATUS_DRAFT;
				$answer->save(false);
			}
			...
			$answerDataProvider = new ActiveDataProvider([
				'query' => $query->where([
					...
					'status' => AnswerInterface::STATUS_PUBLISHED,
				]),
				'pagination' => [
					'pageSize' => 20,
				],
	...
	public function actionDeleteAnswer($id) {
		...
		/** @var Question $model */
		$model = $this->findModel(Answer::className(), $id);
	...
	public function actionAnswer($id) {
		...
		$model->status = AnswerInterface::STATUS_PUBLISHED;
	...
	protected function loadAnswer($id, $break = true) {
		$model = Answer::find()->where('id = :id AND user_id = :user_id', [
			':id' => $id,
			':user_id' => Yii::$app->user->id,
		])->one();
