...
<div class="<?= $class ?>">
	<div class="panel-body">
			...
			<div class="col-md-8 col-xs-6">
				...
				<?php if (Yii::$app->user->getIsAdmin() || Yii::$app->user->getIsEditor()) { ?>
					<div class="edit inline margin-left-10">
						<?= Html::a('['.mb_strtolower(Yii::t('base', 'button_update')).']', ['/project/event/update', 'project_id' => $model->module_id, 'id' => $model->id]) ?>
					</div>
				<?php } ?>
			</div>
		...
		<?php } ?>
		<div class="title inline margin-top-0"><?= Html::a($model->title, ['/projects/event-view', 'project_id' => $model->module_id, 'id' => $model->id], ['target' => $linkTarget]) ?></div>
		<div class="image">
			<?= Html::a(Html::img($model->image->getImageSrc(1000, 400, Mode::RESIZE)), ['/projects/event-view', 'project_id' => $model->module_id, 'id' => $model->id], ['_target' => $linkTarget]) ?>
		</div>
		<div class="text">
			<?= Html::encode($model->descr) ?>
		</div>
		<div class="link">
			<?= Html::a('Читать далее...', ['/projects/event-view', 'project_id' => $model->module_id, 'id' => $model->id], ['_target' => $linkTarget]) ?>
		</div>
	</div>
</div>