...
$this->params['breadcrumbs'][] = ['url' => 'index', 'label' => Yii::t('content-news', 'title')];
		...
		<div class="col-sx-12 col-sm-12 col-md-8 col-lg-9">
				...
				<div class="vote panel panel-default">
					<div class="panel-body">
						<?= \common\modules\vote\widgets\Vote::widget([
							...
							'model' => $model,
							'moduleType' => \common\modules\base\helpers\enum\ModuleType::CONTENT_NEWS,
			...
			<div class="tags margin-bottom-15">
				<?php foreach ($model->tags as $tag) { ?>
					<?= Html::a($tag->title, ['/tags/news', 'title' => $tag->title], ['class' => 'btn btn-primary']) ?>
				<?php } ?>
			</div>
			<?php } ?>
			...
			<?= Html::a('<span class="glyphicon glyphicon-arrow-left"></span> К списку новостей', ['index'], [
				...
				<div class="panel-body">
					<?php if ($model->company_id) { ?>
						<?= $this->render('/companies/default/_block', [
							'model' => $model,
							'url' => ['/companies/default/news', 'id' => $model->company_id],
						]) ?>
					<?php } else { ?>
						<?= $this->render('/author/_block', [
							'model' => $model,
							'url' => ['/user/content/news', 'id' => $model->author_id],
							'viewPayment' => '//news/_view_payment',
						]) ?>
					<?php } ?>
				</div>
			</div>
