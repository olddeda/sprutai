...
reset($paymentTypes);
$currentTypeId = key($paymentTypes);

$js =<<<JS

	var types = $paymentTypesJson;

	var currentTypeId = $currentTypeId;
	...
	if (currentTypeId) {
		$('#payment-payment_type_id').val(currentTypeId).trigger('change');
	}
JS;
$this->registerJs($js);
...
if ($paymentModel->type && $paymentModel->type->price_fixed) {
...
<?php Modal::begin([
	'header' => Html::tag('h4', Yii::t('project', 'popup_payment_title'), ['class' => 'margin-0']),
	'toggleButton' => [
		'label' => Yii::t('project', 'popup_payment_button_open'),
		'tag' => 'button',
