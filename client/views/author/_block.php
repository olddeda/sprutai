...
<?php if ($model->author_id != Yii::$app->user->id && $model->author->profile->getWallet_link()) { ?>
<div class="align-center margin-top-20">
	<a class="btn btn-primary btn-lg" href="<?= $model->author->profile->getWallet_link() ?>" target="_blank">Отблагодарить автора</a>
</div>
<?php } ?>
<?php $testMode = ModulePayment::getInstance()->gateway_current->testMode; ?>
<?php if (1 == 2 && $viewPayment && (($testMode && Yii::$app->user->getIsAdmin()) || (!$testMode && $model->author_id !== Yii::$app->user->id))) { ?>
	<div class="align-center margin-top-20">
		<?= $this->render($viewPayment, [
			'model' => $model,
		]) ?>
	</div>
<?php } ?>
